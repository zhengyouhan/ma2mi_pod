...
## Reparameterization (`reparam.py`)

Sigmoid reparameterization for bounded parameters:
```
x = low + (high - low) · σ(u)
```

- `u ∈ ℝ` (unbounded, learnable)
- `x ∈ [low, high]` (bounded output)
- Gradients flow smoothly even at boundaries (unlike hard clamp)
